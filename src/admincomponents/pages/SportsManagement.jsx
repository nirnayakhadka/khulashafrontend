...
const SportsManagement = () => {
  ...
  const fetchSports = async () => {
    try {
      ...
      showToast('Failed to load sports articles', 'error');
      console.error(err);
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editingId) {
        await axiosInstance.put(`/sports/${editingId}`, formDataToSend, {
          headers: {
            'Content-Type': 'multipart/form-data',
          },
        });
        showToast('Sports article updated successfully!');
      } else {
        await axiosInstance.post('/sports', formDataToSend, {
          headers: {
            'Content-Type': 'multipart/form-data',
          },
        });
        showToast('Sports article created successfully!');
      }
  ...
  const handleEdit = (sports) => {
    ...
    setFormData({
      ...
      publishedDate: sports.publishedDate ? new Date(sports.publishedDate).toISOString().slice(0, 16) : '',
  ...
  const handleDelete = async (id) => {
    ...
    if (!window.confirm('Are you sure you want to delete this sports article?')) return;

    try {
      setLoading(true);
      await axiosInstance.delete(`/sports/${id}`);
      showToast('Sports article deleted successfully!');
      setViewingNews(null);
      fetchSports();
    } catch (err) {
      showToast('Failed to delete sports article', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div className="bg-white rounded-xl shadow-2xl max-w-6xl w-full max-h-[95vh] overflow-y-auto">
      <div className="sticky top-0 bg-white border-b px-6 py-4 flex items-center justify-between z-10">
        <h2 className="text-2xl font-bold text-gray-800">Edit Sports Article</h2>
            ...
            <div>
              ...
              <div
                className={`border-2 border-dashed rounded-lg p-6 text-center cursor-pointer transition ${
                  formData.imagePreview ? 'border-blue-500 bg-blue-50' : 'border-gray-300 hover:border-blue-400'
                }`}
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="flex gap-4 pt-4">
              <button
                ...
                disabled={loading}
                className="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-lg font-medium flex items-center justify-center gap-2 disabled:opacity-60"
              >
                <Save size={18} />
...
{viewingNews && (
  <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div className="bg-white rounded-xl shadow-2xl max-w-6xl w-full max-h-[95vh] overflow-y-auto">
      <div className="sticky top-0 bg-white border-b px-6 py-4 flex items-center justify-between z-10">
        <h2 className="text-2xl font-bold text-gray-800">View Sports Article</h2>
        ...
        <div className="max-w-7xl mx-auto px-6 py-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600">Manage sports news articles</p>
            </div>
            <button
              onClick={() => {
                resetForm();
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        {showForm && (
          <div className="bg-white rounded-xl shadow-xl border mb-10">
            <div className="px-6 py-5 border-b flex items-center justify-between">
              <h2 className="text-2xl font-bold text-gray-800">Create New Sports Article</h2>
        ...
        {/* Articles List - New Responsive Format */}
        {!fetchingData && currentSports.length > 0 && (
          <div className="space-y-4">
            {currentSports.map((sports) => (
              <article
                key={sports.id}
              ...
              >
                ...
                <div className="hidden xl:flex items-start gap-4">
                  ...
                  <div className="flex-shrink-0 overflow-hidden" style={{ width: '200px' }}>
                    <h3
                      className="text-base font-bold text-gray-900 cursor-pointer hover:text-blue-600 transition line-clamp-2"
                      onClick={() => setViewingNews(sports)}
                    >
                      {sports.title}
                    </h3>
                  </div>
                  ...
                  <div className="flex-shrink-0 overflow-hidden" style={{ width: '500px' }}>
                    {sports.subtitle && (
                      <p className="text-sm text-gray-600 line-clamp-2">{sports.subtitle}</p>
                    )}
                  </div>
                  {/* Divider */}
                  <div className="w-px h-20 bg-gray-300 flex-shrink-0"></div>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0" style={{ width: '150px' }}>
                    ...
                    <div className="min-w-0 overflow-hidden">
                      <p className="text-sm font-medium text-gray-900 truncate">{sports.journalistName || 'Unknown'}</p>
                    </div>
                  </div>
                  {/* Divider */}
                  <div className="w-px h-20 bg-gray-300 flex-shrink-0"></div>
                ...
                <div className="md:hidden space-y-3">
                  ...
                  {sports.subtitle && (
                    <>
                      <p className="text-sm text-gray-600">{sports.subtitle}</p>
                      <div className="h-px bg-gray-300"></div>
                    </>
                  )}
                  {/* Journalist Info */}
                  <div className="flex items-center gap-3">
                    ...
                    <p className="text-sm font-medium text-gray-900">{sports.journalistName || 'Unknown'}</p>
                  </div>
                  {/* Horizontal Divider */}
                  <div className="h-px bg-gray-300"></div>
                  {/* Action Buttons */}
        ...
        {/* Empty State */}
        {!fetchingData && sportsList.length === 0 && (
          <div className="text-center py-20 bg-white rounded-xl shadow">
            ...
            <p className="text-gray-500 mb-8">Start by creating your first sports article</p>
            <button
              onClick={() => setShowForm(true)}
              className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-4 rounded-lg font-medium flex items-center gap-2 mx-auto"
            >
              <Plus size={20} />
