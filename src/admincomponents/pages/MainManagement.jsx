...
const fetchNews = async () => {
  try {
    ...
    showToast('Failed to load main news articles', 'error');
    console.error(err);
  } finally {
...
const handleSubmit = async () => {
  ...
  if (!editingId && !formData.newsImageFile && !formData.newsImageUrl) {
    showToast('Please upload a cover image or provide an image URL', 'error');
    return;
  }
  ...
  try {
    ...
    if (editingId) {
      await axiosInstance.put(`/main/${editingId}`, formDataToSend, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
      showToast('Main news updated successfully!');
    } else {
      await axiosInstance.post('/main', formDataToSend, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
      showToast('Main news created successfully!');
    }
...
const handleDelete = async (id) => {
  ...
  if (!window.confirm('Are you sure you want to delete this main news article?')) return;

  try {
    setLoading(true);
    await axiosInstance.delete(`/main/${id}`);
    showToast('Main news article deleted successfully!');
    setViewingNews(null);
    fetchNews();
  } catch (err) {
    showToast('Failed to delete main news article', 'error');
  } finally {
    setLoading(false);
  }
};
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-6xl w-full max-h-[95vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b px-6 py-4 flex items-center justify-between z-10">
              <h2 className="text-2xl font-bold text-gray-800">Edit Main News Article</h2>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="space-y-8">
                  <div>
                    ...
                    <div
                      className={`border-2 border-dashed rounded-lg p-6 text-center cursor-pointer transition ${
                        formData.newsImagePreview ? 'border-indigo-500 bg-indigo-50' : 'border-gray-300 hover:border-indigo-400'
                      }`}
                  ...
                  <div>
                    ...
                    <div
                      className={`border-2 border-dashed rounded-lg p-6 text-center cursor-pointer transition ${
                        formData.journalistImagePreview ? 'border-indigo-500 bg-indigo-50' : 'border-gray-300 hover:border-indigo-400'
                      }`}
                ...
                <div className="lg:col-span-2 space-y-6">
                  ...
                  <div className="flex gap-4 pt-4">
                    <button
                      ...
                      disabled={loading}
                      className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white py-3 rounded-lg font-medium flex items-center justify-center gap-2 disabled:opacity-60"
                    >
                      <Save size={18} />
      ...
      {viewingNews && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-6xl w-full max-h-[95vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b px-6 py-4 flex items-center justify-between z-10">
              <h2 className="text-2xl font-bold text-gray-800">View Main News Article</h2>
                ...
                <div className="lg:col-span-2 space-y-6">
                  ...
                  <div className="flex gap-4 pt-6 border-t">
                    <button
                      ...
                      className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white py-3 rounded-lg font-medium flex items-center justify-center gap-2"
                    >
                      <Edit size={18} />
                      Edit
                    </button>
      ...
      <header className="bg-white shadow">
        <div className="max-w-7xl mx-auto px-6 py-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Main News Management</h1>
              <p className="text-gray-600">Manage featured and headline news stories</p>
            </div>
            <button
              onClick={() => {
                resetForm();
        ...
        {showForm && (
          <div className="bg-white rounded-xl shadow-xl border mb-10">
            <div className="px-6 py-5 border-b flex items-center justify-between">
              <h2 className="text-2xl font-bold text-gray-800">Create New Main News Article</h2>
      ...
      >
        ...
        {!fetchingData && newsList.length === 0 && (
          <div className="text-center py-20 bg-white rounded-xl shadow">
            ...
            <p className="text-gray-500 mb-8">Start by creating your first main news article</p>
            <button
              onClick={() => setShowForm(true)}
              className="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-4 rounded-lg font-medium flex items-center gap-2 mx-auto"
            >
              <Plus size={20} />
